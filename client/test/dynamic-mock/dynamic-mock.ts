...
export function shouldGetRealValue(
  prop: unknown,
  isMockFunctionAccess: boolean,
  lastCalledProp?: unknown,
): boolean {
...
export function mockInterface<T>(): MockedInterface<T> {
  function recurseProxy(
    target: Target<T>,
    isPrevMockFunctionAccess: boolean = false,
    lastCalledProp?: unknown,
  ): MockedInterface<T> {
    return new Proxy(target, {
      get(
        getTarget: Record<string | number | symbol, unknown>,
        ...
        const isMockFunctionAccess =
          isPrevMockFunctionAccess || prop === _IS_MOCK_FUNCTION;
        // Base case:
        ...
        if (!getTarget.hasOwnProperty(prop)) {
          getTarget[prop] = mock.fn();
        }

        // Recurse case:
        // Recursively create proxy object on each get until Base case
        return recurseProxy(
          getTarget[prop] as Target<T>,
          isMockFunctionAccess,
          prop,
        );
      },
