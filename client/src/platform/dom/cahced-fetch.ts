...
export class CachedFetcher implements IFetch {
  ...
  constructor(windowRef: IFetch & ILocalStorage & ICrypto, ttl: number) {
    this.#windowRef = windowRef;
    this.#ttl = ttl;
  }

  async fetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
    const key = await getSHA256Hash(
      this.#windowRef,
      `${input.toString()}/${JSON.stringify(init)}`,
      20,
    );
...
const getSHA256Hash = async (
  windowRef: ICrypto,
  input: string,
  limit?: number,
) => {
  const textAsBuffer = new TextEncoder().encode(input);
  const hashBuffer = await windowRef.crypto.subtle.digest(
    "SHA-256",
    textAsBuffer,
  );
