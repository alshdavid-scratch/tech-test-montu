...
import {
  GiphyService,
  IGiphyService,
} from "../../../platform/giphy/giphy-service.ts";
...
import { rx, forceRender } from "../../../platform/rx/index.ts";
import { useViewModel } from "../../../platform/rx/preact/use-view-model.ts";

export class HomeViewModel {
  #giphyService: IGiphyService;
  #page:
    | AsyncIterableIterator<TrendingResponseData[] | SearchResponseData[]>
    | undefined;
  @rx accessor list: Array<TrendingResponseData | SearchResponseData>;
  @rx accessor loading: boolean;
  @rx accessor showFavorites: boolean;
  @rx accessor searchInput: string;
  @rx accessor favorites: Record<
    string,
    TrendingResponseData | SearchResponseData
  >;

  constructor(giphyService: IGiphyService) {
...
export function HomeView() {
  ...
  const vm = useViewModel(HomeViewModel, [giphyService]);
    ...
    <Fragment>
        ...
        <div className="content-max-width">
          ...
          <div className="quick-menu">
            <Icon
              kind="heart"
              variant="solid"
              className={classNames("show-favorites", [
                "active",
                vm.showFavorites,
              ])}
      ...
      <nav className="search">
        <div className="content-max-width">
          <Input
            ...
            onInput={(e) =>
              (vm.searchInput = (e.target as HTMLInputElement).value)
            }
            onEnter={(_) => vm.search()}
          />

          <button disabled={vm.loading} onClick={() => vm.search()}>
            🔎
          </button>
        </div>
      </nav>
      ...
      <main className={classNames("view-home", ["loaded", !vm.loading])}>
        ...
        <div className="ghosts content-max-width">
          {!vm.showFavorites &&
            Array.from(Array(vm.list.length || 50).keys()).map((_, i) => (
              <div key={i} className="ghost"></div>
            ))}
        </div>

        <div className="images content-max-width">
          {(vm.showFavorites ? Object.values(vm.favorites) : vm.list).map(
            (result) => (
              <div key={result.id} className="image-container">
                <button
                  className={classNames("favorite", [
                    "selected",
                    result.id in vm.favorites,
                  ])}
